<%-- 
    Document   : product
    Created on : Mar 6, 2022, 1:55:28 PM
    Author     : PhatPH
--%>
        ...
        <div class="container-fluid" style="margin: 0;padding: 0;font-family: sans-serif;">
            <div class="row">
                ...
                <c:if test="${not empty sessionScope.SHOP}">
                    <c:forEach var="list" items="${sessionScope.SHOP}">
                        <c:if test="${list.getStatus()== '1'}">
                            <div class="product col-md-4" style=" border: 2px solid;">
                                <form action="BuyController?count=${count}" method="post">
                                    <div class="row" style="display: flex; justify-content: center">
                                        <p><input type="hidden" value="${list.getProID()}" name="productID"></p>
                                        <div class="col-md-6">
                                            <img src="images/${list.getImagePath()}" style="width: 60%">
                                        </div>
                                        <div class="col-md-6">
                                            <p>${list.getProName()}</p> 
                                            <p>Details: ${list.getDescription()}</p>
                                            <p>Price: ${list.getPrice()}</p>  
                                            <input type="submit" value="Add To Cart" style=" width: 100%; padding: 5px;font-size: 25px;"class="btn btn-outline-success"></br>
                                        </div>
                                    </div>
                                </form>
                            </div>
